...
@Slf4j
@Component
public class OrderArrivedNotifier extends OrderCommunicationNotifier {

    @Autowired
    public OrderArrivedNotifier(OrderDeliveryAcceptedNotifier next) {
        super(next);
    }

    @Override
    boolean isResponsible(final OrderCommunicationStepData data) {
        return data.getOrder().getOrderStep().equals(OrderStepEnum.ARRIVED.toString());
    }
