...
@Slf4j
@Component
public class OrderAcceptedNotifier extends OrderCommunicationNotifier {

    @Autowired
    public OrderAcceptedNotifier(OrderInprogressNotifier next) {
        super(next);
    }

    @Override
    boolean isResponsible(final OrderCommunicationStepData data) {
        return data.getOrder().getOrderStep().equals(OrderStepEnum.ACCEPTED.toString());
    }

    @Override
    SendChatbotWhatsappMessageEvent buildEmailEvent(final OrderCommunicationStepData data) {
        //TODO
        return SendChatbotWhatsappMessageEvent.builder()
                .build();
    }
}
